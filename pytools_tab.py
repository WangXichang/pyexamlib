...
def df_to_table(dataframe,
    ...
    total_pageno = int(dataframe.count()[0]/pagelines + 1) \
        if dataframe.count()[0] % pagelines >0 \
        else int(dataframe.count()[0]/pagelines)
    for linenum in range(pagelines, dataframe.count()[0]+pagelines, pagelines):
        ...
        if linenum == pagelines:
            ...
            title = newtitle
        pagenostr = ' ' * int((pagewidth - 8)/2) + '--' + str(pagenum+1) + '--'
        result_string = result_string + newtitle + '\n' + newpage + pagenostr + '\n'
        pagenum += 1
        print(f'{total_pageno}: finished={pagenum}')
    if len(writefilename) > 0:
        fh = open(writefilename, 'w')
