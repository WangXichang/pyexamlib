    ...
    def run(self):
        ...
        for sf in namelist:
            if '_zscore' in sf:
                ...
                self.outdf[newsf] = self.outdf[sf].apply(lambda x: x * self.tscore_std + self.tscore_mean)
...
class TZscoreLinear(ScoreTransformModel):
    ...
    def run(self):
        ...
        for sf in self.scorefields:
            ...
            self.outdf[sf+'_tscore'] = self.outdf[sf+'_zscore'].\
...
class L9score(ScoreTransformModel):
    ...
    def __calcscoretable(self):
        for sf in self.scorefields:
            self.segtable[sf+'_l9score'] = self.segtable[sf+'_percent'].\
